...
class TushareHQChartData(IHQData) :
    ...
    def GetMinuteData(self, symbol, period, right, jobID):
        ...
        stock=data["stock"][0]
        df = pd.DataFrame(stock["minute"])
        print(df)

        cacheData["yclose"]=np.array(df["avprice"]).tolist()
        cacheData["open"]=np.array(df["open"]).tolist()
        cacheData["high"]=np.array(df["high"]).tolist()
        cacheData["low"]=np.array(df["low"]).tolist()
        cacheData["close"]=np.array(df["price"]).tolist()
        cacheData["vol"]=np.array(df["vol"]).tolist()
        cacheData["amount"]=np.array(df["amount"]).tolist()
        cacheData["time"]=np.array(df["time"]).tolist()

        dataCount=len(cacheData["time"]) 
        cacheData['count']=dataCount    # 数据个数

        aryDate=[]
        for item in cacheData["time"] :
            aryDate.append(20210403)
...
def TestSingleStock() :
    ...
    if (res==True) :
        for item in result.Result :
            dfData=HQChartPy2Helper.JsonDataToPandas(item["Data"], item["Symbol"])
            print(dfData)
